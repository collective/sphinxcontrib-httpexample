...
def read_fixture(filename):
    with open(
        ...
        return (lambda s: isinstance(s, bytes) and s or s.encode('utf-8'))(
            fp.read().strip()
        )  # noqa
...
FIXTURE_013_REQUEST = read_fixture('013.request.txt')
FIXTURE_014_REQUEST = read_fixture('014.request.txt')
